...
let puki = ["agatzz", "agat", "AgatZ", "agatdwi"]
...
router.get('/ai/agatai', async (req, res, next) => {
...
router.get('/ai/bard', async (req, res, next) => {
	let text = req.query.text
	if(!text) return res.json(loghandler.nottext)
	fetch(encodeURI(`https://aemt.me/bard?text=${text}`))
...
router.get('/ai/openai', async (req, res, next) => {
	let text = req.query.text
	if(!text) return res.json(loghandler.nottext)
	fetch(encodeURI(`https://aemt.me/openai?text=${text}`))
	.then(response => response.json())
	.then(async data => {
		if(data.status !== true) {
			res.json(loghandler.error)
...
router.get('/download/tiktokslide', async (req, res, next) => {
	let url = req.query.url
	if(!url) return res.json(loghandler.noturl)
	fetch(encodeURI(`https://aemt.me/download/tiktokslide?url=${url}`))
	.then(response => response.json())
	.then(async data => {
...
router.get('/tools/airline', async(req, res) => {
  fetch(encodeURI('https://api.gatzd.biz.id/api/other/cuacabandara?apikey=agatz'))
  .then(response => response.json())
  .then(async data => {
